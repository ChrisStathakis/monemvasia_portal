...
from companies.forms import  CompanyServiceForm, FrontEndCompanyInformationForm
...
def create_new_item_view(request, slug):
    obj = get_object_or_404(Company, slug=slug)
    count_items = obj.my_products.count()
    ...
    if not request.user == obj.owner:
        messages.warning(request, 'ΔΕ ΜΠΟΡΕΙΤΕ ΑΝ ΕΠΕΞΕΡΓΑΣΤΕΙΤΕ ΑΥΤΗ ΤΗΝ ΕΤΑΙΡΙΑ')
        return redirect(reverse('homepage'))
    form = ProductForm(request.POST or None, initial={'company': obj})
    if form.is_valid():
